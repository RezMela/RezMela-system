...
ReserveTouch(key ObjectId, string Data) {
    ...
    while(Ptr--) {
        ...
        ReservedTouchFaces += [ ObjectId, Face ];    
    }
}
...
default {
    ...
    link_message(integer Sender, integer Number, string String, key Id) {
        ...
        // Processing for alternate clicks
            ...
            //Remove data associated with SenderId in the ReservedTouchFaces list
                integer P;
                while ((P = llListFindList(ReservedTouchFaces, [ SenderId ])) > -1) {   // ObjectId is first column of table
                    ReservedTouchFaces = llDeleteSubList(ReservedTouchFaces, P, P + 1);
                }
            ReserveTouch(SenderId, String); //To Add new set of reserved faces
        }
