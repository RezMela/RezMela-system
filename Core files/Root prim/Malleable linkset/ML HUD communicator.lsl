...
string RotateLabel(integer Degrees) {
	string Direction;
	integer Value = Degrees;
	if (Degrees > 0) {
		Direction = "+";
	}
	else {
	...
	return Direction + (string)Value + DEGREES_SYMBOL;
}
RotateObject(string Label) {
	string Sign = llGetSubString(Label, 0, 0);
	integer Degrees = (integer)llGetSubString(Label, 1, -2);    // Numeric part without °(degrees) symbol or preceding +/-
...
ParsePublicData(string Data) {
	...
	string RotationButtonsString = llList2String(Parts, 15);
	if (RotationButtonsString == "") {
		RotationButtons = [ "90", "-90", "45", "-45", "10", "-10", "1", "-1" ];
	}
	else {
		RotationButtons = llCSV2List(RotationButtonsString); // list of strings
	}
