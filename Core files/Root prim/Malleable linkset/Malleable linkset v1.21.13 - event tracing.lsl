...
integer evlink = 0;
getevlink() {
	evlink = osGetLinkNumber("ev");
}
ev(string text) {
	llMessageLinked(evlink, 18181818, text, NULL_KEY);
//	if (text != "") text += "\n" + llGetSubString(llGetTimestamp(), 11, 18);
...
ev("");		
	...
	timer() {
string t = llList2CSV([ UuidLinksInvalid, AwakenTicks, LoadingCompleteTicks, ActivationQueueSize, (LinksToDo != []), AppBackupsWaiting, (AppRestoreData != ""), LogoutTicks, WaitingForCataloguer ]);
ev("timer " + t);		
		llSetTimerEvent(0.0);
...
if (Number == LM_EXECUTE_COMMAND) {
	ev("execute: " + String);
} else {
	ev("link_message " + (string)Number);		
}
		...
		if (Number == LM_TOUCH_NORMAL) {
			...
			key ObjectId = llGetLinkKey(LinkNum);
			if (CpNormal == TOUCH_INVALID_VECTOR) { ev(""); return; }       // they've clicked on a prim edge - ignore
			if (IsMap) {
				if (LinkNum == CursorLinkNum) { ev(""); return; }	// ignore clicks on cursor prim
			...
			if (CPtr > -1) {        // they do
				CPtr -= NOB_AVID;
				if (IsMap) {
					if (LinkNum != BoardLinkNum || TouchFace != BoardFace) { ev(""); return; }	// In map mode, you can only position objects on the control board
				}
				if (DistanceExceeded(TouchPos, FALSE)) { ev(""); return; }		// they've clicked too far from the root
				...
				if (SPtr > -1) {        // they have an object selected
					if (!IsMap) {
						...
						if (ObjectId == SelectedUuid) {    // if they've short-clicked on the selected prim ...
							...
							if (DistanceExceeded(TouchPos, FALSE)) { ev(""); return; }		// they've clicked too far from the root
							integer D = llListFindList(LinkedObjects, [ SelectedUuid ]);
							if (D == -1) { LogError("Can't find prim data for placement"); ev(""); return; }
							D -= LO_OBJECT_ID;
		...
		// Processing for alternate clicks
			...
			if (IsMap) { ev(""); return; }	// long-clicking on linkset prims has no function in Map mode
			key AvId = Id;
			...
			if (MenuPrimName != "") {    // it is a long-click menu prim
				...
				if (ViewOnly && UserId != AvId) { ev(""); return; }	// if view-only is set and they're not signed in, we don't want them selecting prims
				...
				if (FindSelectionByUuid(ObjectId) > -1) { ev(""); return; }        // and stop processing if so
			...
			key SenderId = llGetLinkKey(Sender);
			integer D = llListFindList(LinkedObjects, [ SenderId ]);	
			if (D == -1) { LogError("Can't set extra data"); ev(""); return; }
			D -= LO_OBJECT_ID;
			...
			for (U = 0; U < UuidsCount; U++) {
				...
				if (ObjectReference != "") { // if it's the name of an object in the library
					...
					if (OPtr == -1) { ev(""); return; } // should never happen
...
if (Change & CHANGED_LINK) getevlink();
else ev("changed " + (string)Change);			
