...
list ObjectLibrary;
integer OBJ_NAME			= 0;		// Always first
...
integer OBJ_PHANTOM 		= 21;
integer OBJ_FLOATING 		= 22;
integer OBJ_IS_APP 		= 23;
integer OBJ_STICKPOINT_B64	= 24;
integer OBJ_SNAP_GRID 		= 25;
integer OBJ_REGION_SNAP 	= 26;
integer OBJ_STRIDE 		= 27;
...
//integer RTF_LINKNUM = 0; // memo for reference
...
integer ReadyMessageSent = FALSE;
key OwnerId;
key OurUuid;
...
PositionObjectOnFace(
	...
	if (!IsMap && TargetLink > 0) {
		...
		if (TargetLibPtr > -1) {	// Target is an object
			...
			list SPList = FindStickPoint(TargetLink, StickPoints64, TouchPos, TouchST, TouchFace, Normal);	// Returns VEC_NAN if not found
	...
	CopyRotation = llList2Integer(ObjectLibrary, LibPtr + OBJ_COPY_ROTATION);
...
llOwnerSay(llList2CSV([ llRot2Euler(ObjectRot) * RAD_TO_DEG, llRot2Euler(TargetLocalRot) * RAD_TO_DEG, llRot2Euler(StickPointRot) * RAD_TO_DEG ]));///%%% 
		//ObjectRot *=  (StickPointRot * TargetLocalRot);///%%%
		//ObjectRot *= TargetLocalRot;
		ObjectRot *= StickPointRot;
llOwnerSay(llList2CSV([ llRot2Euler(ObjectRot) * RAD_TO_DEG ]));///%%% 
		//ObjectPos = CpPos + (ObjectOffset * BasicRot * StickPointRot * TargetLocalRot);///%%%
		ObjectPos = CpPos + (ObjectOffset * BasicRot * TargetLocalRot * StickPointRot);
	...
	vector RegionSnap = llList2Vector(ObjectLibrary, LibPtr + OBJ_REGION_SNAP);
	if (IsMap) {
...
list FindStickPoint(integer TargetLink, string StickPoints64, vector TouchPos, vector TouchST, integer TouchFace, vector Normal) {
...
Clone(key AvId) {
	...
	NewObjects += [ AvId, NewObjectsName, NOB_TYPE_CLONE, 0, 
		Pos, Rot, 
		VEC_NAN, -1, VEC_NAN, 
...
AocCreate(key AvId, string Data) {
	...
	vector OriginalSize = ZERO_VECTOR;	// we don't know this yet
	vector ActualSize = ZERO_VECTOR;	// nor this
...
CreateContDetached(
	...
	float SizeFactor) {
		...
		DetachedObjects += [ ObjectId, NULL_KEY, LibPtr, SizeFactor, ExtraData, AppData ];
		ObjectsCount++;
	}
CreateContLinked(
	integer NobPtr,
	...
	float SizeFactor) {
		...
		LinkedObjects += [ NewLinkNum, CpPos, CpNormal, CpBinormal, OriginalSize, ActualSize, SizeFactor, ExtraData ];    // add to LinkedObjects table
		ObjectsCount++;
...
// Actually move prim to specified local position and rotation
...
StoreCatalog(string sData) {
	...
	for (P = 0; P < (OCount - 1); P++) {	// -1 because the last line is empty
		...
		integer Resizable = (integer)		llList2String(Data, 24);
...
AutoHideSet(integer Hide) {
	...
	for (LoPtr = 0; LoPtr < Len; LoPtr += LO_STRIDE) {
		LinkNum = llList2Integer(LinkedObjects, LoPtr + LO_LINKNUM);
		string PrimName = llGetLinkName(LinkNum);
		integer ObjPtr = llListFindList(ObjectLibrary, [ PrimName ]);
		if (ObjPtr > -1) {
			integer AutoHide = llList2Integer(ObjectLibrary, ObjPtr + OBJ_AUTOHIDE);
...
OnRez(integer Param) {
	if (Param == ON_REZ_PARAMETER) {	// if we've been rezzed by an App or Map module
...
list ListModules() {
	...
	for (O = 0; O < Len; O++) {
		...
		integer P = llListFindList(ModulesList, [ LinkKey ]);
		if (P > -1) {
...
list LinkedObjectSaveData(integer LoPtr) {
	integer LinkNum = llList2Integer(LinkedObjects, LoPtr + LO_LINKNUM);
	...
	integer LibPtr = llListFindList(ObjectLibrary, [ ObjectName ]);
	if (LibPtr == -1) { LogError("Can't find library entry to save object data"); return []; }
	integer AutoHide = llList2Integer(ObjectLibrary, LibPtr + OBJ_AUTOHIDE);
...
// Name is the name of the notecard
		...
		for (Lptr = 0; Lptr < LinkedCount; Lptr += LinkedStride) {
			...
			if (CreateGroup) {
				...
				NewObjects += [ AvId, ObjectName, NOB_TYPE_GROUP, 0,
					...
					NewPrimId, ExtraData, OriginalSize, ActualSize, SizeFactor, 0 ];
				ObjectsToLoad++;
...
SelectObject(key AvId, integer LinkNum, key SelId) {
	...
	if (IsMap) {
		...
		if (LOPtr  == -1) { LogError("Can't find object to select"); return; }
		LOPtr -= LO_LINKNUM;
...
SetTimer() {
	...
	if (AwakenTicks || LoadingCompleteTicks || ActivationQueueSize || AppBackupsWaiting || AppRestoreData != "" || LogoutTicks)
...
// If there's a problem with ShuffleLinkNumsGeneric(), check the touch handler and cataloguer too - they have near-copies of this function
...
state Normal {
	...
	state_entry()    {
		// Note that the normal state can be activated twice during the rezzing/loading process!
		//
		...
		SetCursor(FALSE, NULL_KEY, ZERO_VECTOR);	// hide cursor
		LoadingScene = FALSE;
		// If we're a child app, tell our parent we're ready for our data
		// We need to use ReadyMessageSent to prevent duplicate messages being sent because it appears
		// this state can be activated twice during the rezzing/loading process.
		if (ParentId != NULL_KEY && !ReadyMessageSent) {
	...
	timer() {
		...
		if (LogoutTicks > 0) {
			if (--LogoutTicks == 0) {
				...
				// all, there's been time for a user to sign in since we rezzed an initialised.
				if (UserId == NULL_KEY) {
	...
	dataserver(key From, string Data) {
		...
		if (Command == LM_CHILD_READY) {
			...
			ParentId = From;	// store the ID of the Map object
		}
