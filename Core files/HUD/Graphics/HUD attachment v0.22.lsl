...
// HUDA messages (direct to HUD from other scripts)
...
HandleServerMessage(string Command, list Parts) {
	...
	if (Command == HUD_MESSAGE_TITLE) {
		string NewTitle = llList2String(Parts, 0);
	...
	else if (Command == HUD_MESSAGE_CREATE_PAGE) {
		CreatePage(Parts);
	}
...
CreatePage(list Parts) {
	...
	string ExtraParams = llList2String(Parts, 1);
	string CommandList = llList2String(Parts, 2);
	integer LinkNum = InitPage(PageIndex);
	VirtualPages += [ "P", PageIndex, NULL_KEY ];
	VirtualPagesCount++;
	RenderTexture(PageIndex, LinkNum, FACE_PAGE, CommandList, ExtraParams);
...
list PrimLinkTarget(string DebugInfo, integer LinkNum) {
	...
	if (LinkNum <= 0) {
		llDialog(llGetOwner(), "\n\nInvalid link number encountered!\n\nInfo: " + DebugInfo + "\n\n", [ "OK" ], -999999);
...
HandleCameraSet(list Params) {
	if (CameraJumpMode) {
		...
		if (CameraFocus == PreviousCameraFocus) {	// if they've clicked the same object
			...
			else {	// it's not currently the alternate view, so switch to alternate view
				if (CameraAltPos == VEC_NAN) {	// no specific alternate position, so calculate one according to CameraZoomFactor
					CameraAltPos = PreviousCameraPos + ((CameraFocus - PreviousCameraPos) * CameraZoomFactor);
				}
...
Jump(vector Pos, vector LookAt, integer SavePosition) {
	...
	ReleaseCamera();
	// Jump
	osTeleportAgent(OwnerId, Pos, LookAt);
}
CameraJumpSetup() {
...
SetTitle() {
	...
	if (DisplayText == "") DisplayText = "(no title)";
	string CommandList = "";
	// Background
	...
	CommandList = osDrawText(CommandList, DisplayText);
	string ExtraParams = "width:" + (string)TitleCanvasSize + ",height:" + (string)TitleCanvasSize;
	RenderTexture(-1, 1, FACE_TITLE, CommandList, ExtraParams);
...
RenderTexture(integer PageIndex, integer LinkNum, integer Face, string CommandList, string ExtraParams) {
	key TextureId;
	string Hash = llSHA1String(CommandList);
	integer P = llListFindList(HashTable, [ Hash ]);
	if (P == -1) {
		osSetDynamicTextureDataBlendFace("", "vector", CommandList, ExtraParams, FALSE, 2, 0, 0, FACE_RENDER);
		TextureId = llGetTexture(FACE_RENDER);
		HashQueue += [ Hash, TextureId ];
		HashQueueMode = 1;
	}
	else {
...
GetActivateData(list Parts) {
	...
	TitleCanvasSize = 	(integer)	llList2String(Parts, 	3);
	TitleForeColor = 				llList2String(Parts, 	4);
	TitleBackColor = 				llList2String(Parts, 	5);
	TitleFontName = 				llList2String(Parts, 	6);
	TitleFontSize = 	(integer)	llList2String(Parts, 	7);
	TitleLeftMargin = 	(integer)	llList2String(Parts, 	8);
	TitleTopMargin = 	(integer)	llList2String(Parts, 	9);
	RootPrimSize = 		(vector)	llList2String(Parts, 	10);
	PagePrimSize = 		(vector)	llList2String(Parts, 	11);
	PageTextureSize = 	(integer)	llList2String(Parts, 	12);
	PageWidth = 		(integer)	llList2String(Parts, 	13);
	PageHeight = 		(integer)	llList2String(Parts, 	14);
	CameraZoomFactor = 	(float)		llList2String(Parts, 	15);
	UuidLogo = 						llList2String(Parts, 	16);
	UuidMinMax = 					llList2String(Parts, 	17);
	string BackerColorString = 		llList2String(Parts, 	18);
	SplashPrimTexture = 			llList2String(Parts, 	19);
	string FBC = 					llList2String(Parts, 	20);
	FloatBackHeight = 	(float)		llList2String(Parts, 	21);
	string FTCN =					llList2String(Parts, 	22);
	float FTPz = 		(float)		llList2String(Parts, 	23);
	string FTCW =  					llList2String(Parts, 	24);
	// Calculations
...
SetFloatingText() {
	...
	// if the text begins with "!", remove that char and colour the text with the warning colour
