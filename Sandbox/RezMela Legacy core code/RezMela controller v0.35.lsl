// RezMela controller v0.35
//
// v0.35 - added random size/rotation feature
...
Resize(integer Type, integer ObjPtr, key WorldId, key IconId, float AbsoluteSize, integer Remember) {
	...
	if (Rp == -1) {	// if it's not a resizable object
		// Is it a random resizable?
		string Params = GetParams(ObjectName);
		if (!IsParam(Params, "S")) return;	// ignore if it's neither resizable nor randomly sized
	}
	float Size = llList2Float(ObjectData, ObjPtr + OBJ_SIZE);
	if (Size == 0.0) {
		...
		if (Comment != 0) {    // Not a complete comment line
			...
			if (llStringTrim(Line, STRING_TRIM) != "") {    // if there's something left after comments are removed
				...
				if (llGetListLength(L) == 2) {    // so there is a "X = Y" kind of syntax
					...
					else if (Name == "randomsize") {
						float RandomSizeFactor = (float)Value;
						if (RandomSizeFactor > 0.0) {
							Params += "S" + (string)RandomSizeFactor;
						}
					}
					else if (Name == "randomrot") {
						if (String2Bool(Value)) Params += "R";
					}
...
// Hacky as hell, but only a temporary solution until the HUD arrives
integer RandomizeOn() {
	if (ButtonRandomize > -1) {
		string Desc = llList2String(llGetLinkPrimitiveParams(ButtonRandomize, [ PRIM_DESC ]), 0);
...
integer GetLinkNumbers() {
	...
	for(P = 1; P <= PrimCount; P++) {
		...
		else if (Name == "Randomize") ButtonRandomize = P;
...
state Normal {
	...
	dataserver(key From, string Data) {
		...
		if (Command == RZ_PING) {
			...
			if (RandomizeOn()) {
				if (IsParam(ObjectParams, "S")) {	// random size
					// This is a bit hacky. We directly extract the resize parameter from the string, since the design
					// so far has been that the string would just be a series of single-char flags. This could do to be
					// made more elegant. JFH
					integer P = llSubStringIndex(ObjectParams, "S");
					if (P > -1) {
						float RandSizeFactor = (float)llGetSubString(ObjectParams, P + 1, -1);
						RandSizeFactor *= LastResize;		// scale randomness up to current size factor
						Size = LastResize - RandSizeFactor + llFrand(RandSizeFactor * 2.0);
					}
				}
				if (IsParam(ObjectParams, "R")) {	// random rotation
					float Degrees = llFrand(360.0);
					RotateObject(ObjPtr, WorldId, IconId, Degrees);
				}
			}
