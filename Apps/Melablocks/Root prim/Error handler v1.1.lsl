...
// v1.1 - use PHP script to send email
...
ProcessError(key Id, string Text) {
    ...
    for (E = 0; E < EmailsCount; E++) {
        ...
        string Args = Address + "|" + "ERROR " + Text + "|" + EmailText; // To address | Subject | Body
        Args = llEscapeURL(Args);
        string PHP = "http://rezmela.net/php/LogError.php?data=" + Args;
        llHTTPRequest(PHP, [], "");
    }
    llSetTimerEvent(3.0);
}
