...
Display() {
	...
	llOwnerSay("Displaying: " + DisplayText);///%%%
	...
	do {
		...
		for (WordPtr = 0; WordPtr < WordCount; WordPtr++) {
			...
			if (Word == FORMAT_NEWLINE) {
				...
				PotentialLine += Word;
				// Does it overflow?
				llOwnerSay("Comparing 1: " + llList2CSV([ Word, PotentialLine, GetTextWidth(CurrentLine, FontName, CurrentFontSize),  AvailableWidth ]));///%%%
				if (GetTextWidth(PotentialLine, FontName, CurrentFontSize) > AvailableWidth) {
					// Yes, so start a new line for this word
					if (CurrentLine != "") {
						llOwnerSay("Comparing 2: " + llList2CSV([ CurrentLine, GetTextWidth(CurrentLine, FontName, CurrentFontSize),  AvailableWidth ]));///%%%
		...
		if (!ManualFontSize && (TooWide || TotalHeight > AvailableHeight)) {    // if it's too high, drop the font size
			...
			llOwnerSay("    Too big " + (string)CurrentFontSize);///%%%
		}
		else {
			llOwnerSay("    Not too big " + (string)CurrentFontSize);///%%%
			Break = TRUE;
		}
	}
	while (!Break);
	llOwnerSay("Calculated " + (string)CurrentFontSize);///%%%
	integer PosX;
	integer PosY;
	if (CenterHorizontal)
...
default {
	...
	state_entry() {
		...
		OldNumberOfPrims = llGetNumberOfPrims();
		if (OldNumberOfPrims == 1) {
			BlankDisplay();
		}
		else {
			Display();
	...
	changed(integer Change) {
		...
		if (Change & CHANGED_LINK) {
			if (llGetNumberOfPrims() > 1 && OldNumberOfPrims == 1) {	// If we've been linked
