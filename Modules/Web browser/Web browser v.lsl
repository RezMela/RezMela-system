...
// v1.1.1 - add loading screen UUID to config
// v1.1.0 - fix timing issue with LM_LOADING_COMPLETE being received prematurely
...
key LoadingTexture = "e7e5af7a-01e0-4956-a5e2-9755c6dbe87a";
...
Display() {
	...
	llSetTexture(LoadingTexture, FACE_MEDIA);
...
state Normal {
	on_rez(integer Param) { llResetScript(); }
	state_entry() {
		if (FirstTime) { // Processing for first time only
			// This block used to be in state_entry() in the default state, but we found that
			// LM_LOADING_COMPLETE was arriving before the code was complete, and thus being
			// ignored.
